...
CREATE TABLE LECT (
	...
	QNT      INTEGER NOT NULL COMMENT '모집인원', -- 모집인원
	TOTL_HRS DATE    NOT NULL COMMENT '총강의시간', -- 총강의시간
	DAY_HRS  DATE    NOT NULL COMMENT '일강의시간', -- 일강의시간
...
CREATE TABLE STUD (
	...
	WRK     CHAR(1) NOT NULL COMMENT '재직여부' -- 재직여부
)
COMMENT '학생';
...
CREATE TABLE TCHR (
	...
	REUM   VARCHAR(255) NULL     COMMENT '이력서' -- 이력서
)
COMMENT '강사';
...
-- MEMB
ALTER TABLE TABLE16
	ADD CONSTRAINT PK_TABLE16 -- MEMB 기본키
		PRIMARY KEY (
			MNO -- 회원번호
		);
...
-- 학생
ALTER TABLE STUD
	ADD CONSTRAINT FK_TABLE16_TO_STUD -- MEMB -> 학생
		FOREIGN KEY (
			MNO -- 학생번호
		)
...
-- 강사
ALTER TABLE TCHR
	ADD CONSTRAINT FK_TABLE16_TO_TCHR -- MEMB -> 강사
		FOREIGN KEY (
			MNO -- 강사번호
		)
		REFERENCES TABLE16 ( -- MEMB
...
-- 매니저
ALTER TABLE MGR
	ADD CONSTRAINT FK_TABLE16_TO_MGR -- MEMB -> 매니저
		FOREIGN KEY (
			MNO -- 매니저번호
		)
		REFERENCES TABLE16 ( -- MEMB
...
-- 게시판
ALTER TABLE BOARD
	ADD CONSTRAINT FK_TABLE16_TO_BOARD -- MEMB -> 게시판
		FOREIGN KEY (
			MNO -- 회원번호
		)
		REFERENCES TABLE16 ( -- MEMB
...
-- 자료실
ALTER TABLE FBOARD
	ADD CONSTRAINT FK_TABLE16_TO_FBOARD -- MEMB -> 자료실
		FOREIGN KEY (
			MNO -- 회원번호
		)
		REFERENCES TABLE16 ( -- MEMB
...
-- MEMB
ALTER TABLE TABLE16
	ADD CONSTRAINT FK_ADDR_TO_TABLE16 -- 주소 -> MEMB
		FOREIGN KEY (
			ARNO -- 주소번호
		)
		REFERENCES ADDR ( -- 주소
