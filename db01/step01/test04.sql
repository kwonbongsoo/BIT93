...
select lno, titl, mrno                        4
from lect                                    2
where mrno is null                           3
order by titl asc;                            1
...
select lno, titl, mrno, pric
from lect
where pric < 300000 or pric > 500000;
...
select lno, titl, mrno
from lect
where mrno is null; /* mrno = null 은안된다 */
...
=> 컬럼명 like '%aaa'---> 문자열 'aaa'를 포함하는것
=> 컬럼명 like 'aa_bb' --> aa 다음에 한 문자는 어떤 것이라도
된다. 그리고 bb가 온다.
