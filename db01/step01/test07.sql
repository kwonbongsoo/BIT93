...
/* 1) 데이터 입력
=> insert 테이블명(컬럼명, 컬럼명, ...) values(깂, 값 ...)*/


insert memb(mno, name, tel, email, pwd)
values(40, '홍길동1', '1111-1111','hong@test.com', '1111')

/*
=> mno 컬럼 처럼 aoto_increment PK 컬럼인 경우 값을 주지 않아도 된다.
...
=> 사용자 암호처럼 DB 관리자에게 노출되어서는 안되는 값은
   암호화시켜서 저장하라!
   보통 DBMS는 이런 경우를 대비해 암호화 시키는 함수를 제공한다.
   */
...
*/
insert memb
values('홍길동4' , '1111-1111', 'hong4@test.com', password('1111'))
...
=> delete from 테이블명 where 조건
  */
  ...
  delete from memb where mno in (59, 60)

    /*
    => 자식테이블에서 참조하는 데이터인 경우,
       부모테이블에서 삭제할 수 없다.
       데이터가 물려 있으면 삭제할 수 없다...
       물린데이터 하나하나를 다제거해줘야 삭제할수 있다.
       자식테이블 부터  하나씩 제거하란 말이다.
    */
