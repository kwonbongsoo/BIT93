 ...
 *      3) 포트번호 
 *            - OS가 구분할 수 있도록 통신하는 프로그램에 되는 번호.
 *            - 클라이언트쪽 프로그램은 OS가 포트번호를 자동으로 부여한다.
 *            - 서버쪽은 프로그램에서 스스로 지정한다.
 *            단 사용 중인 포트 번호를 중복 사용할 수 없다.
 *            기존에 특정 목적으로 널리 이용되는 포트 번호도 피해야 한다.
 *            예) FTP(21), Telnet(23), SSH(22), SMTP(25), HTTP(80), POP3(110), IMAP4(143), HTTPS(443),
 *                프록시서버(8080), MySql(3306)
 *      4) 대기열 :
...
public class Test01_1 {
  public static void main(String[] args) throws Exception{
    ...
    // 2) 대기열에 있는 클라이언트의 접속을 승인한다.
    // => 만약 대기 중인 클라이언트가 없다면, 리턴 하지 않고 접속할 때 까지 기다린다.
    // => 여러 클라이언트가 대기 중이라면 그 중 첫 번째로 클라이언트와 연결한다.
    ...
    //    그러나 일반적으로 보통 클라이언트에서 먼저 데이터를 전송한다
    //    그래서 서버는 클라이언트의 데이터를 먼저 읽고 그에 대한 응답을 한다.
    int b = in.read(); // 클라이언트에서 1바이트를 보낼 때 까지 계속 기다린다. blocking 모드.
    System.out.printf("%x\n", b);
