 ...
 *         객체 생성시간이 줄어드는 이점이 있다.
 *        - 이점?
 *          1) 메모리 절약(생성된 객체 재사용으로)
 *          2) 객체 생성 시간이 줄어든다. 객체를 준비하는 시간이 줄어든다.
 *          
 * => Pooling 기법을 DB 커넥션 객체를 관리하는데 적용한 것을
 *      "DB Connectrion pool"이라 부른다.
 * 
 * => 작업
 *      1) DBConnectionPool 클래스 작성
 *      2) MemberDao에 DBConnection 적용
 *      3) 테스트
 */
package step22.ex3;
