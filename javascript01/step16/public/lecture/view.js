...
function addSelect() {
  $.getJSON('manager.json', (managers) => { //매니저 셀렉트 옵션 추가.
    for (var s of managers)
      $('<option>').attr('value', s.mrno).text(s.name).appendTo(fiMrno)
  })
  $.getJSON('classroom.json', (classrooms) => { // 강의실 셀렉트 옵션 추가
    for (var s of classrooms)
      $('<option>').attr('value', s.crmno).text(s.name).appendTo(fiCrmno)
  })
}
