...
module.exports = {
  ...
  insert(lecture, successFn, errorFn) {
    this.connection.query(
      'insert into lect(titl,dscp,sdt,edt,qty,pric,thrs,crmno,mrno) \
       values(?,?,?,?,?,?,?,?,?)',
      [lecture.titl, lecture.dscp, lecture.sdt2,
       lecture.edt2, lecture.qty, lecture.pric,
       lecture.thrs, lecture.crmno, lecture.mrno],
  ...
  update(l, successFn, errorFn) {
    this.connection.query(
      ...
      where lno=?',
      [l.titl, l.dscp, l.sdt2, l.edt2, l.qty,
      l.pric, l.thrs, l.crmno, l.mrno, l.no],
      function(error, result) {
        if (error) {
          errorFn(error)
        } else {
