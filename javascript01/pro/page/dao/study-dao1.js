...
module.exports = {
  ...
  search(pageNo, pageSize ,searchValue, type, successFn, errorFn) {
    console.log(type)
    if(type == 'place') {
      this.connection.query(
        "select sno, title, date_format(sdt,'%Y-%m-%d') sdt2 , \
        date_format(edt,'%Y-%m-%d') edt2, many, place \
          from study where place=?\
        order by sdt desc \
        limit ?, ?" ,
  ...
  searchCountAll(searchValue, type, successFn, errorFn) {
    this.connection.query(
      'select count(*) cnt from study',
      function(error, results) {
        if (error) {
